...
`AetherEditor.state` SHALL expose a read-only `EditorStateSnapshot` without a Core-level subscribe or reactive store API. Shells SHALL observe changes through Event Hub `on('change', ...)`.
