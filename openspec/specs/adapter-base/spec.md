...
M3 Adapter baseline and M4 GFM extensions plus M4.5 editor orchestration verification: protocol types and error classes in `@aether-md/core`, Remark and ProseMirror plugin packages, GFM round-trip verification, headless `createEditor` integration tests, and SerializationError placeholder strategy without React Shell or standalone M2 Command Bus automatic rollback.
