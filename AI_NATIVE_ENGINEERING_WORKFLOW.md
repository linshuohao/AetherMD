...
| Full Change  | 是（完整） | 是（plan + 多 task + loop） | 架构 / SDK / workflow semantics / MVP public contract / 多步 |
...
- 开始实现 MVP **public contract** 或可发布 package 的运行时行为。
- `examples/**` demo slice（无 public export / main spec 语义变更）默认 Spec Change（见 `docs/engineering/demo-slice-delivery-program.md`）。
- 修改测试策略或 CI 门禁。
- 废弃、替代或反转 ADR。
