...
| Core 实现者 | [项目状态](project-status.md)、[MVP 实施计划](engineering/mvp-implementation-plan.md) | [产品交互体验规范](architecture/product-experience-spec.md)、[Core API](architecture/core-api.md)、[文档模型](architecture/document-model.md)、[Adapter 协议](engineering/adapter-protocol.md)、[测试策略](engineering/test-strategy.md) |
...
- 目标：L1 Demo Slice 已闭合；**当前推进 L2 Slice A**（`block-morphing-slice-1`）。M7 前置：L1 + **L2 Slice A** 可演示（见 [MVP 实施计划](engineering/mvp-implementation-plan.md)、[ADR 009](adr/009-release-governance.md)）
