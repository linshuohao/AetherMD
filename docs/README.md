...
这里是 AetherMD 的公开文档入口。项目当前处于设计到最小实现过渡阶段，本文档体系用于明确架构边界、插件契约、工程策略和 M1–M4 最小实现基线。
...
- 实现：`@aether-md/core` 已提供 M1 bootstrap、M2 Command/Event 与 M3 document/adapter 类型；`@aether-md/plugin-remark` 与 `@aether-md/plugin-prosemirror` 提供 Adapter 实现；`@aether-md/preset-gfm` 提供 GFM 六语法 round-trip
- 目标：保持 M4 实现、OpenSpec 规格和长期 Docs 一致，再进入 M5 Shell 里程碑
