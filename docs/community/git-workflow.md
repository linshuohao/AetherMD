...
> 本文定义 AetherMD 的分支、提交信息、PR 与合并规则。
...
- 合并前确保 PR 的四个 required checks 全部通过（见下方「约束工具」）。
...
本地 Git hooks（`core.hooksPath=.githooks`，`pnpm install` 时通过 `prepare` 自动安装）：

| Hook         | 作用                                                                      | 跳过方式                                             |
| ------------ | ------------------------------------------------------------------------- | ---------------------------------------------------- |
| `pre-commit` | 对 staged 文件运行 Prettier / ESLint `--fix` 并重新 stage                 | `SKIP_PRE_COMMIT=1 git commit`                       |
| `commit-msg` | 使用 commitlint 校验当前 commit message                                   | `git commit --no-verify`                             |
| `pre-push`   | 校验分支名、commit 范围、并运行 `pnpm prepush`（对齐 CI `Quality gates`） | `SKIP_PRE_PUSH=1 git push` 或 `git push --no-verify` |
...
推送前可单独运行与 CI 相同的 quality job：

```bash
pnpm prepush
```

若只需快速静态检查（不跑全量 test / consumer smoke），可设置 `PRE_PUSH_FAST=1`：
