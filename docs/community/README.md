...
AetherMD 社区协作重点是让设计更清晰、更可审查、更容易落地。

## 页面

| 页面                                                                      | 作用                                                      |
| ------------------------------------------------------------------------- | --------------------------------------------------------- |
| [治理](governance.md)                                                     | 决策归属与维护者职责                                      |
| [审查指南](review-guide.md)                                               | 如何审查架构、SDK 和工程文档                              |
| [Git 工作流规范](git-workflow.md)                                         | 分支、提交信息、PR 与合并规则                             |
| [发布流程](release-process.md)                                            | npm canary / `latest` 发布流程（ADR 009；公开发布时执行） |
...
- 项目许可证：**MIT**（根目录 [LICENSE](../../LICENSE)；O4 已决议，无需改评 Apache-2.0）
- Changelog：**公开发布前**延续 Changesets `changelog: false`；首次 `latest` 前再定手写 `CHANGELOG.md` 或 `@changesets/changelog-github`（O3 已决议）
- 包管理器与 monorepo 工具：pnpm workspace + Turborepo（[ADR 008](../adr/008-repo-toolchain-baseline.md)）
- 发布与版本流程：发布预备已完成；公开发布时启用 Changesets prerelease + CI publish（O1/O2 决议完成前不启动 publish）
