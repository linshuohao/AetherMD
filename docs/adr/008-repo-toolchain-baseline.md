...
在 ADR 009 接受前，本 ADR 不启用 npm publish，不配置 canary release，不添加 release token，不建立 examples matrix。ADR 009 将 publish 与 canary **推迟至 M7** 并定义 M6 预备项；M1–M5 期间本条仍然有效。本 ADR 不改变 `@aether-md/core` public API。
...
- [ADR 009：发布与治理策略](009-release-governance.md)
