...
| 当前目标 | **L1 已闭合**；**L2 Slice A + Slice C 已交付**（`block-morphing-slice-1`、`block-morphing-slice-c`）。**M7** 前置：L1 + L2 可演示 + 维护者 sign-off（见 [ADR 009](adr/009-release-governance.md)） |
...
| **L2 产品交互 north star** | `examples/block-morphing` | ✅ Slice A + Slice C 已交付 | Instant Morphing + multi-block Block Focus（`block-morphing-slice-c`） |
...
1. **下一 slice：** L2 **Slice B** — GFM inline marks 在 source 态保真；见 [产品交互体验规范](architecture/product-experience-spec.md) 与 [MVP 实施计划 — Block Morphing](engineering/mvp-implementation-plan.md#m6-之后纵向-block-morphing活跃)。
2. **M7 发布条件（方案 B）：** L1 `react-basic` 可演示 **且** L2（Slice A + Slice C）可演示 **且** 维护者 sign-off；闭合 ADR 009 O1/O2 后再启动 Release CI / `NPM_TOKEN`。
3. **后置 backlog：** History / Selection / Clipboard — Slice B 之后独立 Spec Change。
