...
| 字段 | 值 |
| --- | --- |
| 阶段 | 设计草案 + M1 Core Bootstrap + M2 Command/Event Runtime |
| 实现 | `@aether-md/core` 已提供 M1 bootstrap 与 M2 Command/Event 运行时 |
...
- Markdown parse/serialize、Remark、ProseMirror、GFM preset

## 近期重点

1. 稳定文档体系。
2. 持续审查 SDK 契约与已实现 Core 边界（M1 bootstrap、M2 Command/Event）。
...
6. 在进入 Adapter、Markdown 或 Shell 里程碑前，继续保持 OpenSpec、Docs 和实现同步。
