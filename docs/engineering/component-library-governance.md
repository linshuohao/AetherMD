...
> 本文定义 AetherMD 作为 NPM 包型 Monorepo、引擎库和 SDK 库长期演进时的治理规则。
...
- **当前基线规则**：当前 `@aether-md/core` 最小实现和文档变更已经需要遵守。
...
| Example / Playground Package | 不发布                       | 不允许                                                   | 可选         | 可选         | 不需要                 | 允许依赖 public package | 允许                           | 允许                     | `examples/react`、playground                                          |
...
- 当前 `@aether-md/core` 仍为 `private: true` 时，不代表可以随意扩大 public API；已暴露的入口仍按 public API 审查。
...
- 当前阶段默认只允许少量能力进入 Stable，例如已经实现并由 OpenSpec 覆盖的 Core Bootstrap surface。
...
- Public API 变更必须有 Changeset；当前 `.changeset/` 只作为 版本影响记录底座，不代表启用正式 npm publish。
...
正式发布前，AetherMD 使用 Changesets 作为版本影响记录机制。当前 `.changeset/` 已启用为 版本影响记录底座，只用于记录 public API、package metadata、workspace package 边界和 SemVer 影响；进入对外发布阶段后，才允许将 Changesets 接入正式 npm publish 流程。
...
当前最小 CI 质量门禁只运行 `pnpm install --frozen-lockfile`、`pnpm check` 和 `pnpm build`。该门禁不包含 npm publish、canary release、release token、examples matrix 或发布自动化。Canary 与 CI publish 的启用时机见 [ADR 009](../adr/009-release-governance.md)（公开发布时）。
...
- Examples matrix。
- Canary release。（[ADR 009](../adr/009-release-governance.md)：公开发布时启用）
- Package ownership。
