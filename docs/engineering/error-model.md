...
`@aether-md/core` 当前导出可实例化的 `CoreError`、`PluginError`、`AdapterError` 与 `SerializationError` 类，均实现 `AetherError`。`RenderError` 仍属 v1.0 完整目标范围。
...
Command/Event Runtime 仅实现 Plugin Handler 错误边界：handler 抛错时转换为 `PluginError`，不向宿主抛出，不要求 Adapter 事务回滚。
...
- degraded `SerializationError`：可实例化并 export；Serializer happy-path 为主。

当前**不**实现 Command Bus 自动 rollback 或 `transactionFailed` auto emit。Adapter 快照语义由 Adapter contract tests 验证。

## GFM Preset baseline

`@aether-md/plugin-remark` Serializer 实现 GFM 失败策略：
...
当前**不**实现 Command Bus 自动 rollback 或 `transactionFailed` auto emit。

`RenderError` 仍属 v1.0 完整目标范围。

## Core Bootstrap baseline
...
- **Repeated bootstrap dispose**：`CoreBootstrapRuntime.dispose()` 重复调用为 no-op，不 throw（与 Command/Event runtime dispose 幂等独立）。

## Command/Event Runtime baseline
...
当前**不**实现 Adapter 事务回滚或 `transactionFailed` 自动 emit。
