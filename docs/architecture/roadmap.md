...
M1–M6 按架构层的里程碑叙事仍保留于上表。**当前排期**以用户可感知的纵向切片为主，执行计划见 [Demo Slice 交付计划](../engineering/demo-slice-delivery-program.md)（PR0 → PR A → PR B，**已闭合**）。
...
| 切片 | 范围 | 状态 |
| --- | --- | --- |
| **纠偏规格** | `product-experience-spec`、north star 分层 L1/L2 | 进行中（`align-instant-morphing-north-star`） |
