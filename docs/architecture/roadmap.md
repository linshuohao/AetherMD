...
> 状态：设计草案 + M1–M5 里程碑已实现（M4 GFM preset 六语法 round-trip、M4.5 headless `createEditor`、M5 `@aether-md/react` React Shell 基线）。本页作为对应主题的维护入口。
...
| 里程碑 | 范围 | 决策依据 |
| --- | --- | --- |
| M6 验证套件 | 契约测试、`examples/headless-gfm`、SDK examples 可编译 | [MVP 实施计划](../engineering/mvp-implementation-plan.md) |
| M7 首次发布与生态 | MIT/LICENSE 元数据同步、npm canary、Release CI、`examples/react-basic` | [ADR 009](../adr/009-release-governance.md) |
