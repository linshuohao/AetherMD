...
│   ├── headless-gfm/
│   ├── react/                       # unified showcase: content + morphing
│   └── vue/
```

当前实现状态：

- `packages/core` — `@aether-md/core`：按能力域分子目录（`bootstrap/`、`manifest/`、`command-event/`、`document/`、`morphing/` 契约、`editor/` 编排）。
- `packages/plugins/plugin-remark` / `plugin-prosemirror` — Adapter 实现（当前规模保持 flat `src/`）。
- `packages/preset-gfm` — `createGfmPreset()`；`morphing/` 策略 + `serialization/` 作为 remark 序列化单一出口。
- `packages/react` — `shell/`（Root/Content/hook/GateLock）+ `morphing/`（产品面）；公开 export 含 `shouldApplyControlledValue`。
...
- `examples/react` — 统一 showcase（content 模式 + morphing 模式）。
- `packages/vue` — 规划项。
...
| `packages/core`                       | 已建立   | 承载 `@aether-md/core`（bootstrap、Command/Event、document/adapter types）                                        |
...
| `packages/react`                      | 已建立   | `@aether-md/react`；React Shell 公开 API；依赖 `@aether-md/core` 与 `@aether-md/plugin-prosemirror`；peer `react` |
...
| 规划包         | 建立条件                                                |
| -------------- | ------------------------------------------------------- |
| `packages/vue` | React Shell 边界已稳定，且 Vue 适配不需要改变 Core 语义 |
