...
> 状态：M1 Core Bootstrap、M2 Command/Event Runtime、M3 document-model / adapter-base 类型与 M4.5 editor orchestration 已实现；M5 React Shell 在 `@aether-md/react`（非 Core export）。本页定义宿主应用调用 `@aether-md/core` 的 v1.0 目标公开入口，并记录当前已实现子集。
...
Core 尚未实现：完整 Guard 链、compile-layer Schema 合并、Permission enforce、`bootstrapCore` Adapter plugin 自动加载、`EditorConfig.logger` 宿主注入（M4.5 内部 stub 可用）。React Shell 由 `@aether-md/react` 提供（M5 基线），不属于 Core API export。
...
以下决策在 M4.5 editor orchestration 与 M5 React Shell 实现期间已冻结为 v1.0 Core API 约束；若实现偏离，**MUST** 在对应 OpenSpec change 的 `design.md` 中记录 deviation。
