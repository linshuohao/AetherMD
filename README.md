...
- 当前目标：L1 架构管线 demo 已闭合；**L2 Slice A–D 已交付**。**M7** 发布工程就绪（consumer smoke、Release CI、去 `private`）；待维护者 sign-off + `NPM_TOKEN` 后首次 canary publish（见 [ADR 009](docs/adr/009-release-governance.md)）

## 安装（M7 canary，待 publish 后可用）

```bash
npm install @aether-md/core@canary @aether-md/react@canary @aether-md/preset-gfm@canary \
  @aether-md/plugin-remark@canary @aether-md/plugin-prosemirror@canary
```

五包为 Changesets `linked` 组，版本同步。首次公开发布目标版本 **`0.1.0`**（0.x 预稳定；能力子集见 [项目状态](docs/project-status.md#v10-差距)）。
...
Phase 1 覆盖 `examples/block-morphing`（19 tests）与 `examples/react-basic`（3 tests）：smoke、Block Focus、Instant Morphing、GateLock、场景 A/B/C、Slice B inline marks、编辑隔离、click/Tab 焦点、逐键打字、序列化探针、`moveBlock` 身份、编辑器稳定性。CI 以独立非阻塞 job `e2e-playwright` 运行（22 tests）。详见 [测试策略](docs/engineering/test-strategy.md)。
