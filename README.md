...
- 当前目标：L1 架构管线 demo 已闭合；**L2 Slice A–D 已交付**（`block-morphing-slice-1` … `block-morphing-slice-d`）。**M7** 前置：L1 + L2 可演示 + 维护者 sign-off（见 [ADR 009](docs/adr/009-release-governance.md)）
