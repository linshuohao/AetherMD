...
import {
  ...
  waitForBlockSynced,
  waitForRendered,
} from "../fixtures/vue-editor";

test.describe("vue block-morphing demo e2e", () => {
  test("smoke: app boots and renders morphing blocks", async ({ page }) => {
    await gotoVueMorphingDemo(page);

    await expect(page.getByTestId("aether-vue-showcase")).toBeVisible();
  ...
  test("morphing mode: no markdown-preview; legacy content mode shows preview", async ({
    page,
  }) => {
    await gotoVueMorphingDemo(page);
    ...
    await expect(page.getByTestId("aether-vue-basic-shell")).toBeVisible();
